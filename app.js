...
// --- 3. الدالة الرئيسية لجلب البيانات (متاحة أو مباعة) ---
async function fetchAndDisplay(folderName, isSold = false) {
    ...
    try {
        ...
        if (!response.ok) return; // تخطي المجلد إذا لم يكن موجوداً
        ...
        files.forEach(async (file) => {
            if (file.name.endsWith('.kml')) {
                ...
                kmlText = kmlText.replace(/http:\/\//g, 'https://'); // إصلاح Mixed Content

                const parser = new DOMParser();
                const kmlDom = parser.parseFromString(kmlText, 'text/xml');
                const kmlLayer = new L.KML(kmlDom);

                // إعدادات الحالة: متاحة vs مباعة
                let popupHtml = "";
                let styleOptions = {};
                let labelClass = "";

                if (isSold) {
                    // إذا كانت الأرض مباعة (أحمر)
                    popupHtml = `
                    ...
                    // إذا كانت الأرض متاحة (بنفسجي/ذهبي)
...
// تشغيل النظام للمجلدين
fetchAndDisplay('kmls', false);      // الأراضي المتاحة
fetchAndDisplay('sold_kmls', true);  // الأراضي المباعة
